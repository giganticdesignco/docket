...
export default defineEventHandler(async (event) => {
  ...
  const [{ data: statuses }, { data: budgets }, { data: projects }, { data: depts }] = await Promise.all([
    ...
    admin.from('projects').select('id, name, lead_id, budget_hours, budget_amount, clients(name)').eq('is_active', true),
  ...
  for (const p of team ?? []) {
    ...
    const [{ data: mine }, { data: busy }, { data: wk }, { data: yd }, { data: avail }, { data: drafts }, { data: waiting }, { data: quotes }] = await Promise.all([
      admin.from('work_items').select('id, title, status, due_on, projects(name, clients(name)), work_item_assignees!inner(user_id)').eq('work_item_assignees.user_id', p.id).not('due_on', 'is', null).lte('due_on', addDays(t, 2)).order('due_on'),
      ...
      admin.from('quotes').select('number, title, valid_until, clients(name)').eq('created_by', p.id).eq('status', 'sent'),
    ])
    ...
    const led = (projects ?? []).filter(pr => pr.lead_id === p.id).map(pr => {
      ...
      return { name: pr.name, client: pr.clients?.name, pct: pct == null ? null : Math.round(pct) }
    }).filter(x => x.pct != null && x.pct >= 80)
    const target = avail?.hours_per_week ?? 30
    const facts = {
      today: t, name: first,
      overdue: overdue.map(w => ({ title: w.title, due: w.due_on, project: w.projects?.name, client: w.projects?.clients?.name, status: label(w.status) })),
      dueToday: dueToday.map(w => ({ title: w.title, project: w.projects?.name, client: w.projects?.clients?.name })),
      dueSoon: dueSoon.map(w => ({ title: w.title, due: w.due_on, project: w.projects?.name })),
      ...
      quotesAwaitingReply: (quotes ?? []).map(qq => ({ number: qq.number, title: qq.title, client: qq.clients?.name, validUntil: qq.valid_until })),
    ...
    if (!dry) {
      ...
      if (p.brief_email) {
        const r = await sendEmail(admin, { to: [p.email], subject: `Your morning brief, ${longDay(t)}`, text: `${text}\n\n${origin}/\n\nTurn this email off on your Account page in Docket.` })
        emailed = r.ok
...
function plainBrief(f: { name: string, overdue: { title: string }[], dueToday: { title: string }[], weeksToReview: string[], lastWeekUnsubmittedEntries: number, quotesAwaitingReply: { number: string }[], meetingsToday: { from: string, to: string }[], hoursThisWeek: number, weeklyTarget: number }) {
  const parts: string[] = []
  if (f.overdue.length) parts.push(`${f.overdue.length} overdue: ${f.overdue.map(w => w.title).slice(0, 4).join(', ')}.`)
  if (f.dueToday.length) parts.push(`Due today: ${f.dueToday.map(w => w.title).join(', ')}.`)
