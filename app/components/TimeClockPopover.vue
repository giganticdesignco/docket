...
const { data: tasks, refresh } = await useAsyncData('clock-my-tasks', async () => {
  ...
  const { data, error } = await supabase
    .from('work_items')
    .select('id, title, status, due_on, estimate_hours, projects(name, clients(name)), work_item_assignees!inner(user_id)')
    .eq('work_item_assignees.user_id', user.value.sub)
    .order('due_on', { ascending: true, nullsFirst: false })
    .limit(60)
  if (error) throw error
  const open = (data ?? []).filter(w => !ws.isDone(w.status) && !ws.isPaused(w.status)).slice(0, 25)
  ...
  <UPopover v-model:open="open" mode="hover" :content="{ side: 'right', align: 'start', sideOffset: 8 }" :open-delay="250" :close-delay="150">
    ...
    <template #content>
      <div class="w-80">
        ...
        <p v-else class="px-3 py-6 text-center text-sm text-muted">Nothing assigned to you.</p>
